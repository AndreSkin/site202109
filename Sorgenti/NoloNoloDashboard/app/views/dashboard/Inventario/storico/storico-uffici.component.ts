...
@Component({
  selector: 'Dashboard-Storico-Clienti',
  templateUrl: './storico-uffici.component.html',
  styleUrls: ['./storico-uffici.component.scss']
})

@Injectable()
export class StoricoUfficiComponent implements OnInit{

  @Input() nome_ufficio: string = '';
  ...
  GetNomeUfficio(){
    ...
    this.nome_ufficio = tmp[4];
  }

  VerificaStorico(id_url: string, json_path: any){
    if (json_path != null){
      for (let i = 0; i < json_path.length; i++){
        if(id_url == json_path[i].office_id) return true;
      }
    }
    return false;
  }

  VerificaStorico2(id_url: string, office_id: string){
    if(id_url == office_id) return true;
    return false;
  }
